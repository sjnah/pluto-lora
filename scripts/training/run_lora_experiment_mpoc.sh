#!/bin/bash
# PLUTO LoRA fine-tuning experiment using MPOC closed-loop outcome filters.
#
# The MPOC filters are generated from fixed pre-FT PLUTO and IDM closed-loop
# rollouts over the existing FT train pool:
#   llm-taxonomy/scripts/experiments/pluto/create_mpoc_filters.py
...
MPOC_FILTER_DIR="${WORKSPACE_ROOT}/llm-taxonomy/artifacts/scenario_filters/pluto_mpoc"
...
if [ "$CURRICULUM_MODE" = "percentile_ehu" ]; then
    METHOD=mpoc METHOD_LABEL="MPOC" bash "${SCRIPT_DIR}/run_lora_experiment_percentile_ehu.sh"
    exit $?
fi
...
CURRICULUM_BASE_EXP="${CURRICULUM_BASE_EXP:-curriculum_lora_mpoc_legacy_raw_${CURRICULUM_VERSION}}"
...
ensure_mpoc_filters() {
    ...
    if [ "$missing" -ne 0 ]; then
        echo ""
        echo "Generate MPOC filters first:"
        echo "  cd ${WORKSPACE_ROOT}/pluto"
        echo "  bash scripts/evaluation/run_mpoc_train_pool_rollouts.sh"
        echo ""
        echo "  cd ${WORKSPACE_ROOT}/llm-taxonomy"
        echo "  python scripts/experiments/pluto/create_mpoc_filters.py \\"
        echo "    --pluto-experiment mpoc_zeroshot_pluto_train \\"
        echo "    --idm-experiment mpoc_idm_train \\"
...
echo "Stage 2/3: MPOC weighted curriculum"
run_lora_train \
    "$STAGE2_EXP" \
    "checkpoint" \
    "$STAGE1_CKPT" \
...
echo "Stage 3/3: MPOC hard-weighted curriculum"
run_lora_train \
    "$STAGE3_EXP" \
    "checkpoint" \
    "$STAGE2_CKPT" \
