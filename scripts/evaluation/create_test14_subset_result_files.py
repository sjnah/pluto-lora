...
def subset_one_experiment(
    ...
    source_metrics = exp_root / source_exp / "metrics"
    dest_dir = exp_root / dest_exp
    dest_metrics = dest_dir / "metrics"

    if not source_metrics.exists():
        raise FileNotFoundError(f"Missing source metrics for {method}: {source_metrics}")

    token_set = set(tokens)
    parquet_files = sorted(source_metrics.glob("*.parquet"))
    if not parquet_files:
        raise FileNotFoundError(f"No parquet metric files under {source_metrics}")

    metric_counts: dict[str, int] = {}
    missing_by_metric: dict[str, list[str]] = {}
    subsets: dict[str, pd.DataFrame] = {}
    for metric_file in parquet_files:
        ...
        if missing:
            missing_by_metric[metric_file.stem] = missing
        subsets[metric_file.name] = subset
        metric_counts[metric_file.stem] = int(len(subset))
    ...
    record = {
        ...
        "source_metrics_dir": str(source_metrics),
        "experiment": dest_exp,
        "label": label,
    ...
    (dest_dir / "log.txt").write_text(
        f"{dest_exp} was generated by slicing {source_exp} full Test14-hard metric parquet files.\n",
        encoding="utf-8",
    )
    return provenance
