...
run_enabled_models() {
    ...
    is_enabled "$RUN_RULE_RAW" && run_model_simulation "Rule-Raw${RULE_RAW_CURRICULUM_VERSION:+ (${RULE_RAW_CURRICULUM_VERSION})}" "$RULE_RAW_CURRICULUM_SLUG" "$RULE_RAW_CKPT" "$filter" "$experiment_suffix" "$scenario_builder" "$run_mode"
